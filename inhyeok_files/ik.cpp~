...
PSHARED_DATA	pSharedMemory;
                ...
                int position_mode_flag = 0;
		int torque_mode_flag = 0;
		int ladder_demo = 0;
...
int FKine_COM(const float *Q_34x1, float *pCOM_3x1)  //input:Q_34x1, output:pCOM_3x1
{
...
int FKine_Hand(char ref_coord, const float *Q_34x1, float *pRH_3x1, float *qRH_4x1, float *pLH_3x1, float *qLH_4x1) // ref_coord=1->pelvis local, ref_coord=0->global 
{
...
int WBIK_DRC_ladder_climbing(unsigned int n)  
{
	...
	if(_FKineUpdatedFlag != 1)
        ...
        //printf("COM %f %f %f\n",des_pCOM_3x1[2], des_pCOM_3x1[1],des_pCOM_3x1[3]);
        //printf("COM %f %f %f\n",_pCOM_3x1[2], _pCOM_3x1[1],_pCOM_3x1[3]);
	if(n<=2)
	{
		...
		meas_Q_34x1[RSP_34] = ((float)RSPEncoderValue);
		meas_Q_34x1[RSR_34] = ((float)RSREncoderValue) + OFFSET_RSR*D2R;
		meas_Q_34x1[RSY_34] = ((float)RSYEncoderValue);
		meas_Q_34x1[REB_34] = ((float)REBEncoderValue) + OFFSET_REB*D2R;
		meas_Q_34x1[RWY_34] = ((float)RWYEncoderValue);
		meas_Q_34x1[RWP_34] = ((float)RWPEncoderValue);
		meas_Q_34x1[RWY2_34] = ((float)RWY2EncoderValue);

		meas_Q_34x1[LSP_34] = ((float)LSPEncoderValue);
		meas_Q_34x1[LSR_34] = ((float)LSREncoderValue) + OFFSET_LSR*D2R;
		meas_Q_34x1[LSY_34] = ((float)LSYEncoderValue);
		meas_Q_34x1[LEB_34] = ((float)LEBEncoderValue) + OFFSET_LEB*D2R;
		meas_Q_34x1[LWY_34] = ((float)LWYEncoderValue);
		meas_Q_34x1[LWP_34] = ((float)LWPEncoderValue);
		meas_Q_34x1[LWY2_34] = ((float)LWY2EncoderValue);
...
*/
	...
	dqLH_4x1[4] = 0.f;
        ...
        //printf("COMor1 %f %f %f\n",dpCOM_3x1[3], pCOM1_3x1[3],des_pCOM_3x1[3]);

	reset_neutral_flag = 0;  
	if(position_mode_flag == 0) // staying
	{	
		t = 0.f; 
	} 
	else if(position_mode_flag == 1)  // move hands
	{	
		
		dpRH_3x1[1] = one_cos(t, dpRH[0], move_sec);
		dpRH_3x1[2] = one_cos(t, dpRH[1], move_sec);
		dpRH_3x1[3] = one_cos(t, dpRH[2], move_sec);

		dpLH_3x1[1] = one_cos(t, dpLH[0], move_sec);
		dpLH_3x1[2] = one_cos(t, dpLH[1], move_sec);
		dpLH_3x1[3] = one_cos(t, dpLH[2], move_sec);

		ftemp1_7x1[0] = one_cos(t, drvRH[0], move_sec);
		ftemp1_7x1[1] = drvRH[1];
		ftemp1_7x1[2] = drvRH[2];
		ftemp1_7x1[3] = drvRH[3];
		RV2QT(ftemp1_7x1, dqRH_4x1);

		ftemp1_7x1[0] = one_cos(t, drvLH[0], move_sec);
		ftemp1_7x1[1] = drvLH[1];
		ftemp1_7x1[2] = drvLH[2];
		ftemp1_7x1[3] = drvLH[3];
		...
		if(t > move_sec)
		{
			...
			position_mode_flag = 0;
		}
	}
	else if(position_mode_flag == 2)  //move RF and LF
	{
		dpRF_3x1[1] = one_cos(t, dpRF[0], move_sec);
		dpRF_3x1[2] = one_cos(t, dpRF[1], move_sec);
		dpRF_3x1[3] = one_cos(t, dpRF[2], move_sec);

		dpLF_3x1[1] = one_cos(t, dpLF[0], move_sec);
		dpLF_3x1[2] = one_cos(t, dpLF[1], move_sec);
		dpLF_3x1[3] = one_cos(t, dpLF[2], move_sec);

		ftemp1_7x1[0] = one_cos(t, drvRF[0], move_sec);
		ftemp1_7x1[1] = drvRF[1];
		ftemp1_7x1[2] = drvRF[2];
		ftemp1_7x1[3] = drvRF[3];
		RV2QT(ftemp1_7x1, dqRF_4x1);

		ftemp1_7x1[0] = one_cos(t, drvLF[0], move_sec);
		ftemp1_7x1[1] = drvLF[1];
		ftemp1_7x1[2] = drvLF[2];
		ftemp1_7x1[3] = drvLF[3];
	...
	else if(position_mode_flag == 3)  //move COM
	{	
		dpCOM_3x1[1] = one_cos(t, dpCOM[0], move_sec);
		dpCOM_3x1[2] = one_cos(t, dpCOM[1], move_sec);
	...
	else if(position_mode_flag == 4)  //move pPELz
	{
		dpPELz = one_cos(t, dpPELz, move_sec);
	...
	else if(position_mode_flag == 5)  //offline traj
	{
		dpPELz = off_traj_dpPELz[off_traj_count];
		dpCOM_3x1[1] = off_traj_dpCOM[off_traj_count][0];
		dpCOM_3x1[2] = off_traj_dpCOM[off_traj_count][1];
		dpRH_3x1[1] = off_traj_dpRH[off_traj_count][0];
		dpRH_3x1[2] = off_traj_dpRH[off_traj_count][1];
		dpRH_3x1[3] = off_traj_dpRH[off_traj_count][2];
		dpLH_3x1[1] = off_traj_dpLH[off_traj_count][0];
		dpLH_3x1[2] = off_traj_dpLH[off_traj_count][1];
		dpLH_3x1[3] = off_traj_dpLH[off_traj_count][2];
		dpRF_3x1[1] = off_traj_dpRF[off_traj_count][0];
		dpRF_3x1[2] = off_traj_dpRF[off_traj_count][1];
		dpRF_3x1[3] = off_traj_dpRF[off_traj_count][2];
		dpLF_3x1[1] = off_traj_dpLF[off_traj_count][0];
		dpLF_3x1[2] = off_traj_dpLF[off_traj_count][1];
		dpLF_3x1[3] = off_traj_dpLF[off_traj_count][2];
		RV2QT(off_traj_drvRH[off_traj_count], dqRH_4x1);
		RV2QT(off_traj_drvLH[off_traj_count], dqLH_4x1);
		RV2QT(off_traj_drvRF[off_traj_count], dqRF_4x1);
		RV2QT(off_traj_drvLF[off_traj_count], dqLF_4x1);
		off_traj_count++;
		if(off_traj_count >= off_traj_length)
		{
			reset_neutral_flag = 1;
			off_traj_count = 0;
			position_mode_flag = 0;
    ...
    //printf("COM2 %f %f %f\n",dpCOM_3x1[2], pCOM1_3x1[2],des_pCOM_3x1[2]);
    //printf("COM1 %f %f %f\n",dpCOM_3x1[1], pCOM1_3x1[1],des_pCOM_3x1[1]);
    //printf("COM3 %f %f %f\n",dpCOM_3x1[3], pCOM1_3x1[3],des_pCOM_3x1[3]);   
    

	des_pCOM_3x1[1] = pCOM1_3x1[1] + dpCOM_3x1[1];
	des_pCOM_3x1[2] = pCOM1_3x1[2] + dpCOM_3x1[2];
	des_pCOM_3x1[3] = pCOM1_3x1[3] + dpCOM_3x1[3];
	...
	Xpel_3x1[1] = kd[9]*(-_Qp_33x1[4]) + kp[9]*ftemp3_7x1[1];
	Xpel_3x1[2] = kd[9]*(-_Qp_33x1[5]) + kp[9]*ftemp3_7x1[2];
	Xpel_3x1[3] = kd[9]*(-_Qp_33x1[6]) + kp[9]*ftemp3_7x1[3];
	//-----------------	

	//------------------ Waist
	RVALS(_Q_34x1[WST_34],  _Qp_33x1[WST_33],  kd[10]*(des_WSTp-_Qp_33x1[WST_33])+kp[10]*(des_WST-_Q_34x1[WST_34]),   WSTpmax, WSTppmax, WSTmin, WSTmax, D2R, &Xwst, &ftemp1_7x1[0], &ftemp1_7x1[1]);
	//-----------------

	//--------- Pelvis height
	Xpelz = kd[11]*(des_vPELz-_Qp_33x1[3]) + kp[11]*(des_pPELz-_Q_34x1[3]);
	//-----------------
	
	//------------ Xcom				
	mult_mv((const float**)_jCOM_3x33,3,33, _Qp_33x1, vCOM_3x1);	
	ftemp2_7x1[1] = kd[12]*(des_vCOM_3x1[1]-vCOM_3x1[1]) + kp[12]*(des_pCOM_3x1[1]-_pCOM_3x1[1]);
	ftemp2_7x1[2] = kd[12]*(des_vCOM_3x1[2]-vCOM_3x1[2]) + kp[12]*(des_pCOM_3x1[2]-_pCOM_3x1[2]);
	ftemp2_7x1[3] = kd[12]*(des_vCOM_3x1[3]-vCOM_3x1[3]) + kp[12]*(des_pCOM_3x1[3]-_pCOM_3x1[3]);
	...
	sum_svsv(kp[13], ftemp3_7x1,3, kd[13],ftemp1_7x1, ftemp3_7x1);
	...
	sum_svsv(kp[13],ftemp3_7x1,3, kd[13],ftemp4_7x1, ftemp2_7x1);
	...
	sum_svsv(kp[14], ftemp3_7x1,3, kd[14],ftemp1_7x1, ftemp3_7x1);
	...
	sum_svsv(kp[14],ftemp3_7x1,3, kd[14],ftemp4_7x1, ftemp2_7x1);
	...
	dim_primary_task = 27;
    //printf("%f %f %f %f %f %f\n",X1_33x1[13],X1_33x1[14],X1_33x1[15],X1_33x1[16],X1_33x1[17],X1_33x1[18]);
     
	//----------------- redundant tasks
	RVALS(_Q_34x1[WST_34],  _Qp_33x1[WST_33],  kd[15]*(-_Qp_33x1[WST_33])+kp[15]*(neutral_Q_34x1[WST_34]-_Q_34x1[WST_34]),	WSTpmax, WSTppmax, WSTmin, WSTmax, D2R, &X2_33x1[1], &ftemp1_7x1[0], &ftemp1_7x1[1]);
	RVALS(_Q_34x1[RSP_34],  _Qp_33x1[RSP_33],  kd[15]*(-_Qp_33x1[RSP_33])+kp[15]*(neutral_Q_34x1[RSP_34]-_Q_34x1[RSP_34]),   RSPpmax, RSPppmax, RSPmin, RSPmax, D2R, &X2_33x1[2], &ftemp1_7x1[0], &ftemp1_7x1[1]);
	RVALS(_Q_34x1[RSR_34],  _Qp_33x1[RSR_33],  kd[15]*(-_Qp_33x1[RSR_33])+kp[15]*(neutral_Q_34x1[RSR_34]-_Q_34x1[RSR_34]),   RSRpmax, RSRppmax, RSRmin, RSRmax, D2R, &X2_33x1[3], &ftemp1_7x1[0], &ftemp1_7x1[1]);
	RVALS(_Q_34x1[RSY_34],  _Qp_33x1[RSY_33],  kd[15]*(-_Qp_33x1[RSY_33])+kp[15]*(neutral_Q_34x1[RSY_34]-_Q_34x1[RSY_34]),   RSYpmax, RSYppmax, RSYmin, RSYmax, D2R, &X2_33x1[4], &ftemp1_7x1[0], &ftemp1_7x1[1]);
	RVALS(_Q_34x1[REB_34],  _Qp_33x1[REB_33],  kd[15]*(-_Qp_33x1[REB_33])+kp[15]*(neutral_Q_34x1[REB_34]-_Q_34x1[REB_34]),   REBpmax, REBppmax, REBmin, REBmax, D2R, &X2_33x1[5], &ftemp1_7x1[0], &ftemp1_7x1[1]);
	RVALS(_Q_34x1[RWY_34],  _Qp_33x1[RWY_33], kd[15]*(-_Qp_33x1[RWY_33])+kp[15]*(neutral_Q_34x1[RWY_34]-_Q_34x1[RWY_34]),   RWYpmax, RWYppmax, RWYmin, RWYmax, D2R, &X2_33x1[6], &ftemp1_7x1[0], &ftemp1_7x1[1]);
	RVALS(_Q_34x1[RWP_34],  _Qp_33x1[RWP_33],  kd[15]*(-_Qp_33x1[RWP_33])+kp[15]*(neutral_Q_34x1[RWP_34]-_Q_34x1[RWP_34]),   RWPpmax, RWPppmax, RWPmin, RWPmax, D2R, &X2_33x1[7], &ftemp1_7x1[0], &ftemp1_7x1[1]);
	RVALS(_Q_34x1[LSP_34],  _Qp_33x1[LSP_33],  kd[15]*(-_Qp_33x1[LSP_33])+kp[15]*(neutral_Q_34x1[LSP_34]-_Q_34x1[LSP_34]),   LSPpmax, LSPppmax, LSPmin, LSPmax, D2R, &X2_33x1[8], &ftemp1_7x1[0], &ftemp1_7x1[1]);
	RVALS(_Q_34x1[LSR_34],  _Qp_33x1[LSR_33],  kd[15]*(-_Qp_33x1[LSR_33])+kp[15]*(neutral_Q_34x1[LSR_34]-_Q_34x1[LSR_34]),   LSRpmax, LSRppmax, LSRmin, LSRmax, D2R, &X2_33x1[9], &ftemp1_7x1[0], &ftemp1_7x1[1]);
	RVALS(_Q_34x1[LSY_34],  _Qp_33x1[LSY_33],  kd[15]*(-_Qp_33x1[LSY_33])+kp[15]*(neutral_Q_34x1[LSY_34]-_Q_34x1[LSY_34]),   LSYpmax, LSYppmax, LSYmin, LSYmax, D2R, &X2_33x1[10], &ftemp1_7x1[0], &ftemp1_7x1[1]);
	RVALS(_Q_34x1[LEB_34],  _Qp_33x1[LEB_33],  kd[15]*(-_Qp_33x1[LEB_33])+kp[15]*(neutral_Q_34x1[LEB_34]-_Q_34x1[LEB_34]),   LEBpmax, LEBppmax, LEBmin, LEBmax, D2R, &X2_33x1[11], &ftemp1_7x1[0], &ftemp1_7x1[1]);
	RVALS(_Q_34x1[LWY_34],  _Qp_33x1[LWY_33],  kd[15]*(-_Qp_33x1[LWY_33])+kp[15]*(neutral_Q_34x1[LWY_34]-_Q_34x1[LWY_34]),   LWYpmax, LWYppmax, LWYmin, LWYmax, D2R, &X2_33x1[12], &ftemp1_7x1[0], &ftemp1_7x1[1]);
	RVALS(_Q_34x1[LWP_34],  _Qp_33x1[LWP_33],  kd[15]*(-_Qp_33x1[LWP_33])+kp[15]*(neutral_Q_34x1[LWP_34]-_Q_34x1[LWP_34]),   LWPpmax, LWPppmax, LWPmin, LWPmax, D2R, &X2_33x1[13], &ftemp1_7x1[0], &ftemp1_7x1[1]);
	RVALS(_Q_34x1[RWY2_34],  _Qp_33x1[RWY2_33],  kd[15]*(-_Qp_33x1[RWY2_33])+kp[15]*(neutral_Q_34x1[RWY2_34]-_Q_34x1[RWY2_34]),   RWY2pmax, RWY2ppmax, RWY2min, RWY2max, D2R, &X2_33x1[14], &ftemp1_7x1[0], &ftemp1_7x1[1]);
	RVALS(_Q_34x1[LWY2_34],  _Qp_33x1[LWY2_33], kd[15]*(-_Qp_33x1[LWY2_33])+kp[15]*(neutral_Q_34x1[LWY2_34]-_Q_34x1[LWY2_34]),   LWY2pmax, LWY2ppmax, LWY2min, LWY2max, D2R, &X2_33x1[15], &ftemp1_7x1[0], &ftemp1_7x1[1]);
  ...
  //_jT1_33x33[30][6] = 1.f;
	...
	if(pinv_SR((const float**)_jT1_33x33, dim_primary_task, 33,  0.0000000001, _jT1inv_33x33) != 0)
...
//printf("va = %d %f %f\n",RSY_34,_Q_34x1[RSY_34],_Qp_33x1[RSY_33]);
	int ii=0;
	do
	{       //ii++;
		isLimited = 0;
		if(torque_mode_flag == 0 || torque_mode_flag == 3)
			for(i=1;i<=27;i++)
			{
				index_limited[i] = 0;
				qpp_limited[i] = 0.f;
...
RHPRefAngleCurrent=0*DEG2RAD;
RKNRefAngleCurrent=0*DEG2RAD;
RAPRefAngleCurrent=0*DEG2RAD;
RARRefAngleCurrent=0*DEG2RAD;
LHYRefAngleCurrent=0*DEG2RAD;
LHRRefAngleCurrent=0*DEG2RAD;
LHPRefAngleCurrent=0*DEG2RAD;
LKNRefAngleCurrent=0*DEG2RAD;
LAPRefAngleCurrent=0*DEG2RAD;
LARRefAngleCurrent=0*DEG2RAD;

WSTRefAngleCurrent=0*DEG2RAD;
RSPRefAngleCurrent=0*DEG2RAD;
RSRRefAngleCurrent=0*DEG2RAD;
RSYRefAngleCurrent=0*DEG2RAD;
REBRefAngleCurrent=0*DEG2RAD;
RWYRefAngleCurrent=0*DEG2RAD;
RWPRefAngleCurrent=0*DEG2RAD;
LSPRefAngleCurrent=0*DEG2RAD;
LSRRefAngleCurrent=0*DEG2RAD;
LSYRefAngleCurrent=0*DEG2RAD;
LEBRefAngleCurrent=0*DEG2RAD;
LWYRefAngleCurrent=0*DEG2RAD;
LWPRefAngleCurrent=0*DEG2RAD;
RWY2RefAngleCurrent=0*DEG2RAD;
LWY2RefAngleCurrent=0*DEG2RAD;

drvRF[3]=0;drvRF[2]=0;drvRF[1]=0;drvRF[0]=0;
drvLF[3]=0;drvLF[2]=0;drvLF[1]=0;drvLF[0]=0;
dpRF[2]=0;dpRF[1]=0;dpRF[0]=0;
dpLF[2]=0;dpLF[1]=0;dpLF[0]=0;
drvRH[3]=0;drvRH[2]=0;drvRH[1]=0;drvRH[0]=0;
drvLH[3]=0;drvLH[2]=0;drvLH[1]=0;drvLH[0]=0; 
dpRH[2]=0;dpRH[1]=0;dpRH[0]=0;
dpLH[2]=0;dpLH[1]=0;dpLH[0]=0;
dpCOM[2]=0;dpCOM[1]=0;dpCOM[0]=0; 
dpPELz=0; 
drvPEL[3]=0;drvPEL[2]=0;drvPEL[1]=0;drvPEL[0]=0;
...
FILE *fp;
FILE *fp3;
/*if ( (fp =fopen( "ladder_climbing_data.txt","r")) == NULL ){
		{
...
/*
if ( (fp =fopen( "gain_cloop_ladder.txt","r")) == NULL ){
		{
			printf("Data file 'gain_cloop_ladder.txt' was not found.");		
			return 0;
		}
		for(int i=0;i<17;i++)
		{
			fscanf(fp,"%f %f", &(kp[i]), &(kd[i]));
			while(fgetc(fp) != '\n');
		}
                fclose(fp);}
*/

kp[0]=1500,	kd[0]=20;       // RH px
kp[1]=1500,	kd[1]=20	;// RH py
kp[2]=1500,	kd[2]=20	;// RH pz
kp[3]=20,	kd[3]=0.8	;// RH att
kp[4]=1500,	kd[4]=20	;// LH px
kp[5]=1500,	kd[5]=20	;// LH py
kp[6]=1500,	kd[6]=20	;// LH pz
kp[7]=20,	kd[7]=0.8	;// LH att
kp[8]=200,	kd[8]=0.8	;// redundant task, in force control mode
kp[9]=800,	kd[9]=60	;// Xpel, in kinematics
kp[10]=400,	kd[10]=40	;// Xwst, in kinematics
kp[11]=400,	kd[11]=40	;// Xpelz, in kinematics
kp[12]=900,	kd[12]=60	;// Xcom, in kinematics
kp[13]=900,	kd[13]=60	;// Xfoot, in kinematics
kp[14]=900,	kd[14]=60	;// Xhand, in kinematics
kp[15]=100,	kd[15]=20	;// redundant task, in kinematics0
kp[16]=0,	kd[16]=0	;//
...
printf("update_INIT %d %d\n\n",_FKineUpdatedFlag,_PassiveUpdatedFlag);}


while(_OfflineCount<=16){ 


/*while(_OfflineCount<=int(move_sec/DT)){ 
*/
...
float ref[27];
 ...
 ref[14] = RSREncoderValue=RSRRefAngleCurrent*DEG2RAD- OFFSET_RSR*D2R;
 ref[15] = RSYEncoderValue=RSYRefAngleCurrent*DEG2RAD;
 ref[16] = REBEncoderValue=REBRefAngleCurrent*DEG2RAD- OFFSET_REB*D2R;
 ...
 ref[20] = LSREncoderValue=LSRRefAngleCurrent*DEG2RAD- OFFSET_LSR*D2R;
 ref[21] = LSYEncoderValue=LSYRefAngleCurrent*DEG2RAD;
 ref[22] = LEBEncoderValue=LEBRefAngleCurrent*DEG2RAD- OFFSET_LEB*D2R;
...
fprintf(fp3, "%f %f %f %f %f %f %f %f %f %f %f %f    %f     %f %f %f %f %f %f %f %f %f %f %f %f %f %f",ref[0],ref[1],ref[2],ref[3],ref[4],ref[5],ref[6],ref[7],ref[8],ref[9],ref[10],ref[11],ref[12],ref[13],ref[14],ref[15],ref[16],ref[17],ref[18],ref[19],ref[20],ref[21],ref[22],ref[23],ref[24],ref[25],ref[26]);
fprintf(fp3,"\n");
_OfflineCount++;
