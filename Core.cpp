...
{	
	...
	static char encoder_flag;

	if(ReadSensorFlag != 0x00)
	{
	...
	case CTRLMODE_WALKING:
		...
		{	
			...
			pSharedMemory->Data_Debug[0][pSharedMemory->Data_Debug_Index] = WalkingInfo[RIGHT][X].RefPatternCurrent;// + WalkingInfo[RIGHT][X].RefPosFF + WalkingInfo[RIGHT][X].ControlDSPZMP;
			pSharedMemory->Data_Debug[1][pSharedMemory->Data_Debug_Index] = WalkingInfo[RIGHT][Y].RefPatternCurrent;// + WalkingInfo[RIGHT][Y].RefPosFF + WalkingInfo[RIGHT][Y].ControlDSPZMP;
			pSharedMemory->Data_Debug[2][pSharedMemory->Data_Debug_Index] = WalkingInfo[RIGHT][Z].RefPatternCurrent;//Joint[RHP].RefAngleCurrent;//WalkingInfo[RIGHT][Z].RefPatternCurrent;
			pSharedMemory->Data_Debug[3][pSharedMemory->Data_Debug_Index] = WalkingInfo[LEFT][X].RefPatternCurrent;//Joint[RKN].RefAngleCurrent;//WalkingInfo[LEFT][X].RefPatternCurrent + WalkingInfo[RIGHT][X].RefPosFF + WalkingInfo[RIGHT][X].ControlDSPZMP;//Joint[RHR].ControlAngleFF;
			pSharedMemory->Data_Debug[4][pSharedMemory->Data_Debug_Index] = WalkingInfo[LEFT][Y].RefPatternCurrent;//Joint[RAP].RefAngleCurrent;//WalkingInfoSway.RefPatternCurrent;//Joint[LHR].ControlAngleFF;
			pSharedMemory->Data_Debug[5][pSharedMemory->Data_Debug_Index] = WalkingInfo[LEFT][Z].RefPatternCurrent;//Joint[RAR].RefAngleCurrent;//WalkingInfo[LEFT][Z].RefPatternCurrent;//WalkingInfo[RIGHT][Z].RefPosFF;
			pSharedMemory->Data_Debug[6][pSharedMemory->Data_Debug_Index] = Joint[LHY].RefAngleCurrent;//StepChangeFlag;
			pSharedMemory->Data_Debug[7][pSharedMemory->Data_Debug_Index] = Joint[LHR].RefAngleCurrent;//tempStep;
			pSharedMemory->Data_Debug[8][pSharedMemory->Data_Debug_Index] = Joint[LHP].RefAngleCurrent;//lastStep;
			pSharedMemory->Data_Debug[9][pSharedMemory->Data_Debug_Index] = Joint[LKN].RefAngleCurrent;//WalkingInfo[RIGHT][Y].ControlDSPZMP;
			pSharedMemory->Data_Debug[10][pSharedMemory->Data_Debug_Index] = Joint[LAP].RefAngleCurrent;//WalkingInfo[LEFT][Z].RefPatternCurrent;//Joint[LAP].RefAngleCurrent;//lastStep3;
			pSharedMemory->Data_Debug[11][pSharedMemory->Data_Debug_Index] = Joint[LAR].RefAngleCurrent;//Joint[LAR].RefAngleCurrent;//FTSensor[LFFT].Pitch;
			
			pSharedMemory->Data_Debug[12][pSharedMemory->Data_Debug_Index] = sinf(Joint[LHY].RefAngleCurrent*PI/180);//IMUSensor[CENTERIMU].Roll;
			pSharedMemory->Data_Debug[13][pSharedMemory->Data_Debug_Index] = cosf(Joint[LHY].RefAngleCurrent*PI/180);//IMUSensor[CENTERIMU].Pitch;
			pSharedMemory->Data_Debug[14][pSharedMemory->Data_Debug_Index] = IMUSensor[CENTERIMU].Roll_Velocity;
			pSharedMemory->Data_Debug[15][pSharedMemory->Data_Debug_Index] = IMUSensor[CENTERIMU].Pitch_Velocity;

			pSharedMemory->Data_Debug[16][pSharedMemory->Data_Debug_Index] = FTSensor[RFFT].Mx;
			pSharedMemory->Data_Debug[17][pSharedMemory->Data_Debug_Index] = FTSensor[RFFT].My;
			pSharedMemory->Data_Debug[18][pSharedMemory->Data_Debug_Index] = FTSensor[RFFT].Fz;
			pSharedMemory->Data_Debug[19][pSharedMemory->Data_Debug_Index] = FTSensor[RFFT].Filtered_Mx;
			pSharedMemory->Data_Debug[20][pSharedMemory->Data_Debug_Index] = FTSensor[RFFT].Filtered_My;
			pSharedMemory->Data_Debug[21][pSharedMemory->Data_Debug_Index] = FTSensor[RFFT].Filtered_Fz;

			pSharedMemory->Data_Debug[22][pSharedMemory->Data_Debug_Index] = FTSensor[LFFT].Mx;
			pSharedMemory->Data_Debug[23][pSharedMemory->Data_Debug_Index] = FTSensor[LFFT].My;
			pSharedMemory->Data_Debug[24][pSharedMemory->Data_Debug_Index] = FTSensor[LFFT].Fz;
			pSharedMemory->Data_Debug[25][pSharedMemory->Data_Debug_Index] = FTSensor[LFFT].Filtered_Mx;
			pSharedMemory->Data_Debug[26][pSharedMemory->Data_Debug_Index] = FTSensor[LFFT].Filtered_My;
			pSharedMemory->Data_Debug[27][pSharedMemory->Data_Debug_Index] = FTSensor[LFFT].Filtered_Fz;
			
			pSharedMemory->Data_Debug[28][pSharedMemory->Data_Debug_Index] = ZMP[0];
			pSharedMemory->Data_Debug[29][pSharedMemory->Data_Debug_Index] = ZMP[1];
			pSharedMemory->Data_Debug[30][pSharedMemory->Data_Debug_Index] = ZMP[2];
			pSharedMemory->Data_Debug[31][pSharedMemory->Data_Debug_Index] = ZMP[3];
			pSharedMemory->Data_Debug[32][pSharedMemory->Data_Debug_Index] = ZMP[4];
			pSharedMemory->Data_Debug[33][pSharedMemory->Data_Debug_Index] = ZMP[5];

			pSharedMemory->Data_Debug[34][pSharedMemory->Data_Debug_Index] = WalkingStartStop;
			pSharedMemory->Data_Debug[35][pSharedMemory->Data_Debug_Index] = supportPhase;
			pSharedMemory->Data_Debug[36][pSharedMemory->Data_Debug_Index] = landingState;
			pSharedMemory->Data_Debug[37][pSharedMemory->Data_Debug_Index] = WalkingPhaseNext[Z];
			pSharedMemory->Data_Debug[38][pSharedMemory->Data_Debug_Index] = ZMP[4];
			pSharedMemory->Data_Debug[39][pSharedMemory->Data_Debug_Index] = ZMP[5];
	...
	if(ReadSensorFlag == 0x01) 
	{
		...
		if(encoder_flag != 0)
		{
			RBrequestEncoderPosition(CAN1, Joint[LSP].JMC, 0x00);
			RBrequestEncoderPosition(CAN1, Joint[LSY].JMC, 0x00);
			RBrequestEncoderPosition(CAN1, Joint[LWY].JMC, 0x00);
			RBrequestEncoderPosition(CAN1, Joint[LWY2].JMC, 0x00);
			RBrequestEncoderPosition(CAN1, Joint[NKY].JMC, 0x00);
			encoder_flag = 0;
		}
		else if(encoder_flag == 0)
		{
			RBrequestEncoderPosition(CAN1, Joint[RSP].JMC, 0x00);
			RBrequestEncoderPosition(CAN1, Joint[RSY].JMC, 0x00);	
			RBrequestEncoderPosition(CAN1, Joint[RWY].JMC, 0x00);
			RBrequestEncoderPosition(CAN1, Joint[RWY2].JMC, 0x00);
			encoder_flag = 1;
		}
...
{	
	...
	case CTRLMODE_CCTM_CDI:	// by CDIs
		...
		for(i=0; i<6; i++)
		{
			ctm.th_old[i] = ctm.th[i];
		}

		for(i=12; i<=17; i++)	// RSP:12, RSR:13, RSY:14, REB:15, RWY:16, RWP:17
		{
			Joint[i].RefAngleCurrent = (float)(Joint[i].EncoderValue/Joint[i].PPR);

			// Homesearch Result Compensation
			if(i==13)
			{
				ctm.th[i-12] = (float)(Joint[i].RefAngleCurrent*DEG2RAD - 0.262f);	// -15 deg
			}
			else if(i==15)
			{
				ctm.th[i-12] = (float)(Joint[i].RefAngleCurrent*DEG2RAD - 0.174f);	// -10 deg
			}
			else
			{
				ctm.th[i-12] = (float)(Joint[i].RefAngleCurrent*DEG2RAD);
			}
		}

		for(i=0; i<6; i++)
		{
			ctm.thd[i] = (float)( (ctm.th[i] - ctm.th_old[i])/(INT_TIME1/1000.0f) );
		}

		// Calculate CTM
		Hubo2RightArmCCTM(&ctm);

		if(ctm.pwmsend_flag==1)
		{
			// PWM Digit Setting
			// RSP:12, RSR:13
			Joint[RSP].PWMOUT = (int)(ctm.CTMOUT_LPF[0]);
			//Joint[RSP].PWMOUT = (int)(ctm.Gcp[0] + ctm.Tau_u[0]);
			Joint[RSP].PWMOUTFlag = 0x01;

			Joint[RSR].PWMOUT = (int)(ctm.CTMOUT_LPF[1]);
			//Joint[RSR].PWMOUT = (int)(ctm.Gcp[1] + ctm.Tau_u[1]);
			Joint[RSR].PWMOUTFlag = 0x01;

			// RSY:14, REB:15
			Joint[RSY].PWMOUT = (int)(ctm.CTMOUT_LPF[2]);
			//Joint[RSY].PWMOUT = (int)(ctm.Gcp[2] + ctm.Tau_u[2]);
			Joint[RSY].PWMOUTFlag = 0x01;

			//Joint[REB].PWMOUT = (int)(ctm.CTMOUT_LPF[3]);	// Strange only this value uu
			Joint[REB].PWMOUT = (int)(ctm.CTMOUT_raw[3]);	
			//Joint[REB].PWMOUT = (int)(ctm.Gcp[3] + ctm.Tau_u[3]);
			Joint[REB].PWMOUTFlag = 0x01;
			
			// RWY:16, RWP:17
			Joint[RWY].PWMOUT = (int)(ctm.CTMOUT_LPF[4]);
			//Joint[RWY].PWMOUT = (int)(ctm.Gcp[4] + ctm.Tau_u[4]);
			Joint[RWY].PWMOUTFlag = 0x01;

			Joint[RWP].PWMOUT = (int)(ctm.CTMOUT_LPF[5]);
			//Joint[RWP].PWMOUT = (int)(ctm.Gcp[5] + ctm.Tau_u[5]);
			Joint[RWP].PWMOUTFlag = 0x01;
		}

		/*
		if(ctm_timeIndex>=200)
		{
			printf("\n>> ----------------------------------------------------------------------------");
			printf("\n>> PWM DIGIT Gravi : RSP=%.3f, RSR=%.3f, RSY=%.3f, REB=%.3f, RWY=%.3f, RWP=%.3f", ctm.Gcp[0], ctm.Gcp[1], ctm.Gcp[2], ctm.Gcp[3], ctm.Gcp[4], ctm.Gcp[5]);
			printf("\n>> PWM DIGIT Tau_u : RSP=%.3f, RSR=%.3f, RSY=%.3f, REB=%.3f, RWY=%.3f, RWP=%.3f", ctm.Tau_u[0], ctm.Tau_u[1], ctm.Tau_u[2], ctm.Tau_u[3], ctm.Tau_u[4], ctm.Tau_u[5]);
			printf("\n>> PWM DIGIT raw : RSP=%.3f, RSR=%.3f, RSY=%.3f, REB=%.3f, RWY=%.3f, RWP=%.3f", ctm.CTMOUT_raw[0], ctm.CTMOUT_raw[1], ctm.CTMOUT_raw[2], ctm.CTMOUT_raw[3], ctm.CTMOUT_raw[4], ctm.CTMOUT_raw[5]);
			printf("\n>> PWM DIGIT old : RSP=%.3f, RSR=%.3f, RSY=%.3f, REB=%.3f, RWY=%.3f, RWP=%.3f", ctm.CTMOUT_old[0], ctm.CTMOUT_old[1], ctm.CTMOUT_old[2], ctm.CTMOUT_old[3], ctm.CTMOUT_old[4], ctm.CTMOUT_old[5]);
			printf("\n>> PWM DIGIT LPF : RSP=%.3f, RSR=%.3f, RSY=%.3f, REB=%.3f, RWY=%.3f, RWP=%.3f", ctm.CTMOUT_LPF[0], ctm.CTMOUT_LPF[1], ctm.CTMOUT_LPF[2], ctm.CTMOUT_LPF[3], ctm.CTMOUT_LPF[4], ctm.CTMOUT_LPF[5]);
			printf("\n>> PWM DIGIT Out : RSP=%d, RSR=%d, RSY=%d, REB=%d, RWY=%d, RWP=%d", Joint[RSP].PWMOUT, Joint[RSR].PWMOUT, Joint[RSY].PWMOUT, Joint[REB].PWMOUT, Joint[RWY].PWMOUT, Joint[RWP].PWMOUT);
			printf("\n>> X act : x=%.3f, y=%.3f, z=%.3f", ctm.X_act[0], ctm.X_act[1], ctm.X_act[2]);
			printf("\n>> TH act: th1=%.3f, th2=%.3f, th3=%.3f, th4=%.3f, th5=%.3f, th6=%.3f", ctm.th[0], ctm.th[1], ctm.th[2], ctm.th[3], ctm.th[4], ctm.th[5]);
			printf("\n>> TH dot: thd1=%.3f, thd2=%.3f, thd3=%.3f, thd4=%.3f, thd5=%.3f, thd6=%.3f", ctm.thd[0], ctm.thd[1], ctm.thd[2], ctm.thd[3], ctm.thd[4], ctm.thd[5]);
			printf("\n>> PWM DIGIT F_u   : x=%.3f, y=%.3f, z=%.3f, REB=%.3f, RWY=%.3f, RWP=%.3f", ctm.Fu[0], ctm.Fu[1], ctm.Fu[2]);
			printf("\n>> FT Sensor: Mx=%.3f, My=%.3f, Fz=%.3f", FTSensor[RWFT].Mx, FTSensor[RWFT].My, FTSensor[RWFT].Fz);
			printf("\n>> FT Sensor Integral: X=%.3f, Y=%.3f, Z=%.3f", ctm.RFT_XYZ_LPF[0], ctm.RFT_XYZ_LPF[1], ctm.RFT_XYZ_LPF[2]);
		...
		*/
		...
		for(i=12; i<=17; i++)
		{
			Hubo2JointPWMSignChange(Joint[i]);
		}
		...
		// Encoder Requeset
		RBrequestEncoderPosition(CAN1, JMC8, 0x00);		// RSP, RSR
		RBrequestEncoderPosition(CAN1, JMC9, 0x00);		// RSY, REB
		RBrequestEncoderPosition(CAN1, EJMC0, 0x00);	// RWY, RWP
		break;
	case CTRLMODE_WB_OFFLINE_DEMO: // by Inhyeok
...
int WholeBodyOffline_Demo(unsigned char canChannel)		// by Inhyeok
{
	...
	if(canChannel == CAN0) 
	{
		...
		case WB_QUAD_TEST:			
			...
			break;
		case WB_STEERING_TEST:		
			i_temp = WBIK_DRC_steering(_OfflineCount);			
			break;
		case WB_LADDER_CLIMBING_TEST:
